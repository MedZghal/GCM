...
$(function() {
				...
				if(id==="Presente"){
                                    if(listeAttend.indexOf(eventObjMenu.id)=== -1 && eventObjMenu.type!=="0"){
                                        ...
                                        if(Err.toString()==="true"){
                                            ...
                                            window.parent.toastr.error("Attention,Ce Patient est Déja existe dans la Liste!.",'Error',option);
                                        
                                    }else{
                                        if(listeAttend.indexOf(eventObjMenu.id)!== -1)
                                            window.parent.toastr.error('Ce Patient est Déja existe dans la Liste!.','Error',option);
                                        else
    ...
    scheduler.attachEvent("onEventDeleted",function(id){
       var Err=SuppRdv(id);
        if(Err===true)
            window.parent.swal("Notification !", "Rendez-Vous Supprimer Avec Succès. ", "success");
        else
